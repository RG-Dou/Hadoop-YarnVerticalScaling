...
public class AMRMClientImpl<T extends ContainerRequest> extends AMRMClient<T> {
  ...
  public AllocateResponse allocate(
          ...
          List<ContainerResourceChangeRequest> decreaseRequests) throws IOException, YarnException {
    AllocateResponse response = null;

//    for(ContainerResourceChangeRequest request : increaseRequests){
//      ContainerId containerId = request.getContainerId();

//    }

    AllocateRequest req = AllocateRequest.newInstance(lastResponseId, 0F, null, null,
            null, increaseRequests, decreaseRequests);
    response = rmClient.allocate(req);
